...
# for ra tables, must conform to tbeptools format
chldattab <- chldat %>% 
  rename(
...
```{r}
#| label: fig-boxplot
#| fig-cap: !expr paste0(params$maxyr, " monthly chlorophyll-a bay segment means (red dots) compared to monthly distributions from prior years (box plots and black dots).  Prior years extend to 1975 for OTB, HB, MTB, and LTB, 1991 for BCBS, 1989 for TCB, and 1990 for MR. Boxes encompass the 25th and 75th percentiles, while whiskers bound the interquartile range. Dots beyond the whiskers represent outliers throughout the 1972-", params$maxyr -1, " sample period.")
#| fig-height: 15
#| fig-width: 12
#| fig-align: center
...
#| fig-align: center
#| out-width: "40%"
# show_chlmatrix(chldat, params$maxyr, fml)
```

```{r}
#| label: tbl-raotb
#| tbl-cap: !expr ratabcap('OTB')
show_ratab(bay_segment = 'OTB', yrsel = params$maxyr, epcdata = chldattab)
```

```{r}
#| label: tbl-rahb
#| tbl-cap: !expr ratabcap('HB')
show_ratab(bay_segment = 'HB', yrsel = params$maxyr, epcdata = chldattab)
```

```{r}
#| label: tbl-ramtb
#| tbl-cap: !expr ratabcap('MTB')
show_ratab(bay_segment = 'MTB', yrsel = params$maxyr, epcdata = chldattab)
```

```{r}
#| label: tbl-raraltb
#| tbl-cap: !expr ratabcap('RALTB')
show_ratab(bay_segment = 'RALTB', yrsel = params$maxyr, epcdata = chldattab)
```
